...
bool LogMonitor::prepare_log(MLog *m) 
{
  ...
  wait_for_finished_proposal(new C_Log(this, m, channel_last));
  return true;
}
...
void LogMonitor::_updated_log(MLog *m)
{
  ...
  // send a map of highest seq per channel (to be used by monitors supporting
  // channels), along with the highest seq as the MLogAck's 'last', for those
  // monitors not supporting multiple channels -- this keeps backward compat.
  map<string,version_t> channel_last;
  version_t highest_seq = 0;
  for (deque<LogEntry>::iterator p = m->entries.begin();
       p != m->entries.end(); ++p) {
    version_t &v = channel_last[p->channel];
    if (v < p->seq)
      v = p->seq;
    if (highest_seq < p->seq)
      highest_seq = p->seq;
  }
  mon->send_reply(m, new MLogAck(m->fsid, highest_seq, channel_last));
